...
class U
{
    ...
    public Tree balanced_tree_(Tree a,
                               ...
                               Tree d)
    {
        return this.tree(true, this.tree(false, a, k1, v1, b), k2, v2, this.tree(false, c, k3, v3, d));
    }
...
class Tree
{
    ...
    public Tree insert_(int k, int v)
    {
        ...
        {
            if (k < idx)
                t = ((new U()).tree(color,left.insert_(k, v), idx, val, right)).balance();
            else
            if (idx < k)
                t = ((new U()).tree(color, left, idx, val, right.insert_(k, v))).balance();
            else
        ...
        if (this.is_nonempty_black())
            if (left.is_nonempty_red())
                if ((left.left()).is_nonempty_red())
                {
                    ...
                    v3 = val;
                    d = right;
                    t = (new U()).balanced_tree_(a, k1, v1, b, k2, v2, c, k3, v3, d);
                }
                else
                    t = this.balance_();
            else
